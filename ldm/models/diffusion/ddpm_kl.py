...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.kl_loss_flag = kl_loss_flag
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, sampled_concept= None):
        ...
        if hasattr(self, "split_input_params"):
            ...
            x_recon = self.model(x_noisy, t, sampled_concept = sampled_concept, **cond)
    ...
    def dis_loss(self, model_forward, x_t, t, cond, sampled_concept):
        ...
        # logits_deta = (logits_deta - logits_deta.min(-1)[0][:,None])/(logits_deta.max(-1)[0][:,None] - logits_deta.min(-1)[0][:,None])
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        loss = 0.
        if self.kl_loss_flag:
            loss += self.kl_weight * self.cond_stage_model.kl_loss(self.model.diffusion_model.latent_unit).mean()
            del self.cond_stage_model.posteriors
        ...
        loss_rec = loss_simple / torch.exp(logvar_t) + logvar_t
        # loss = loss_simple / torch.exp(self.logvar) + self.logvar

        loss += self.l_simple_weight * loss_rec.mean()
    ...
    def log_images(self, batch, N=8, n_row=8, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
                   ...
                   plot_diffusion_rows=True, plot_swapped_concepts = False, plot_decoded_xstart=False, **kwargs):
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, sampled_concept = None, c_concat: list = None, c_crossattn: list = None):
