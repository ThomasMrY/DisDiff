...
import numpy as np
import torch
from evaluate_med_wopca import eval_func
from multiprocessing import  Process

csv_name = "results_detail"
repre_type = "dis_repre"
# for orginal version
...
def evaluate_process(new_eval_path, eval_path):
    ...
    data_array = np.load(new_eval_path.replace("checkpoints/",f"{repre_type}/").replace(".ckpt",".npz"))
    latents = torch.from_numpy(data_array["latents"])
    if not os.path.exists(os.path.join(logdir,"dis_metrics_med_wopca")):
        os.makedirs(os.path.join(logdir,"dis_metrics_med_wopca"))
    org_results = eval_func(label_dataset, latents, os.path.join(logdir, "dis_metrics_med_wopca"), int(data_array['num_samples']))
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            files = [npz_file.replace(f"{repre_type}/","checkpoints/").replace(".npz",".ckpt") for npz_file in npz_files]
            # if len(df) == 0:
            #     record_files = []
            # else:
            ...
            if len(mat_files) == 0:
                ...
                record_files = [mapping_func(mat.replace("dis_metrics_med_wopca/","checkpoints/")) for mat in mat_files]
